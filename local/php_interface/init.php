<?
    ...
    class AfterElementAdd
    {
        ...
        {
            //ÐŸÑ€Ð¾Ð²ÐµÑ€Ð¸Ð»Ð¸ Ð½Ð¾Ð¼ÐµÑ€ Ð¸Ð½Ñ„Ð¾Ð±Ð»Ð¾ÐºÐ°
            if($arFields["IBLOCK_ID"] == 24)
            {

                //Ð’Ñ‹Ð±Ñ€Ð°Ð»Ð¸ Ð½ÑƒÐ¶Ð½Ñ‹Ðµ ÑÐ²Ð¾Ð¹ÑÑ‚Ð²Ð°
                $arEventFields = array(
                ...
                //ÐžÑ‚Ð¿Ñ€Ð°Ð²Ð¸Ð»Ð¸ Ð½ÑƒÐ¶Ð½Ð¾Ðµ Ð¿Ð¸ÑÑŒÐ¼Ð¾ Ñ Ð²Ñ‹ÑˆÐµÑƒÐºÐ°Ð·Ð°Ð½Ð½Ñ‹Ð¼Ð¸ Ð´Ð°Ð½Ð½Ñ‹Ð¼Ð¸
    ...
    //Ð´Ð¾Ð±Ð°Ð²Ð»ÑÐµÐ¼ Ð² Ð¿Ð¸ÑÑŒÐ¼Ð¾ Ð¾ Ð·Ð°ÐºÐ°Ð·Ðµ Ð´Ð¾Ð¿Ð¾Ð»Ð½Ð¸Ñ‚ÐµÐ»ÑŒÐ½ÑƒÑŽ Ð¸Ð½Ñ„Ð¾Ñ€Ð¼Ð°Ñ†Ð¸ÑŽ
        ...
        //function orderDataChange($order_id, &$arFields, &$arTemplate)
        {
            ...
            if ($arFields["ORDER_ID"] > 0) {
                if ($_SERVER["HTTP_HOST"]=="shaveclub.ru"){
                    //Ð¾Ð±Ñ‰Ð°Ñ Ð¸Ð½Ñ„Ð¾ Ð¾ Ð·ÐºÐ°Ð·Ðµ
                    $order = CSaleOrder::GetById($arFields["ORDER_ID"]);


                    //ÑÐ»ÑƒÐ¶Ð±Ð° Ð´Ð¾ÑÑ‚Ð°Ð²ÐºÐ¸
                    ...
                    //Ð¿Ð»Ð°Ñ‚ÐµÐ¶Ð½Ð°Ñ ÑÐ¸ÑÑ‚ÐµÐ¼Ð°
                    $paysystem = CSalePaysystem::GetById($order["PAY_SYSTEM_ID"]);
                    //ÑƒÐ±Ð¸Ñ€Ð°ÐµÐ¼ Ð»Ð¸ÑˆÐ½Ð¸Ðµ ÑÐ¸Ð¼Ð²Ð¾Ð»Ñ‹ Ð² Ñ†ÐµÐ½Ðµ
                    $pattern = "/(\D)/";
                    ...
                    $arFields["ORDER_LIST"] = $basketItem["NAME"].' - '.round($basketItem["QUANTITY"]).' ÑˆÑ‚.: '.round($basketItem["PRICE"]).' Ñ€ÑƒÐ±.';
                    if ($arFields['DELIVERY_PRICE']=='Ð‘ÐµÑÐ¿Ð»Ð°Ñ‚Ð½Ð¾') {
   ...
   /**
   * Ðàçðåøåíèå äîñòàâêè ïðè ñìåíå ñòàòóñà çàêàçà íà "Ïåðåäàí â ÑÄ" èëè "Âîçâðàò"
   */
    ...
    /**
    * Ïåðåêëþ÷åíèå ôëàãà îïëàòû çàêàçà â çíà÷åíèå "Äà" ïðè ñìåíå ñòàòóñà çàêàçà íà "Âûïîëíåí"
    */
    ...
    function updatingPaymentProp ($ID, $ar_fields) {
        if ($ar_fields["STATUS_ID"] == "F") {
            ...
            //åñëè ôëàã îïëàòû íå ñòîèò - ñòàâèì
            if ($order["PAYED"] != "Y") {
    ...
    /**
    * îáðàáîòêà ñòàòóñîâ çàêàçà ïðè ïîëó÷åíèè îïëàòû
    */
    AddEventHandler('sale', 'OnSalePayOrder', "updOrderStatus");
    
    function updOrderStatus ($ID, $val) {
        //ïðè ïîëó÷åíèè îïëàòû
        if ($val == "Y") {
            $order = CSaleOrder::GetById($ID);
            //åñëè òåêóùèé ñòàòóñ çàêàíà - íå "Âûïîëíåí", ñòàâèì ñòàòóñ "íîâûé, îïëà÷åí"
    ...
    /**
    * Ïåðåêëþ÷åíèå ôëàãà îòãðóçêè â çíà÷åíèå "Äà" ïðè ñìåíå ñòàòóñà çàêàçà íà "Âûïîëíåí"
    */
    ...
    function SentMail(&$arFields, &$arTemplate) {
        ...
        if ($arOrder_new["quick_order"] && $arOrder_new["quick_order"] != "Y") {
            ...
            $arOrderProps = array();
            $order_props = CSaleOrderPropsValue::GetOrderProps($ORDER_ID);//Ð¡Ð²Ð¾Ð¹ÑÑ‚Ð²Ð° Ð·Ð°ÐºÐ°Ð·Ð°
            ...
            //email Ð¸Ð· Ð½Ð°ÑÑ‚Ñ€Ð¾ÐµÐº ÑŽÐ·ÐµÑ€Ð°
            ...
            if (IntVal($arFields["DELIVERY_PRICE"]) <= 0) {
                $arFields["DELIVERY_PRICE"] = 'Ð‘ÐµÑÐ¿Ð»Ð°Ñ‚Ð½Ð¾';
            }
            ...
            if (!empty($arBasketId)) {
                ...
                foreach ($arBasketValue as $arBasket) {
                    ...
                    if ($arBasket["QUANTITY"] > 1) {
                        $ItemPrice = round($arBasket["QUANTITY"] * $arBasket["PRICE"], 0) . ' (' . round($arBasket["PRICE"], 0) . '/ÑˆÑ‚.)';
                    }
                    ...
                    <span style=\"font-family: HelveticaBold; font-size: 16px !important;\">".round($arBasket["QUANTITY"], 0)." ÑˆÑ‚.</span>
                    </div>
                    </td>
                    <td width=\"230px\">
    ...
    //Ð´Ð»Ñ ÐºÐ°Ð¶Ð´Ð¾Ð³Ð¾ ÑÐ°Ð¹Ñ‚Ð° init.php ÑÐ²Ð¾Ð¹ Ð¸ Ð½Ð°Ñ…Ð¾Ð´Ð¸Ñ‚ÑÑ Ð² Ð¿Ð°Ð¿ÐºÐµ, ÑÐ¾Ð¾Ñ‚Ð²ÐµÑ‚ÑÑ‚Ð²ÑƒÑŽÑ‰ÐµÐ¹ ID ÑÐ°Ð¹Ñ‚Ð° (s1 Ð¸Ð»Ð¸ s2) Ð¸ Ð²ÐµÑÑŒ ÑƒÐ½Ð¸ÐºÐ°Ð»ÑŒÐ½Ñ‹Ð¹ Ð´Ð»Ñ ÑÐ°Ð¹Ñ‚Ð° ÐºÐ¾Ð´ Ð¿Ð¸ÑÐ°Ñ‚ÑŒ Ñ‚ÑƒÐ´Ð°
