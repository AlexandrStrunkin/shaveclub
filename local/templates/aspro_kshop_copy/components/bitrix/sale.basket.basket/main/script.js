                ...
                $(oldBasket).find(".tabs_content.basket li").each(function(i, element)
                    {					
                        if (!$(newBasket).find(".tabs_content.basket li:eq("+i+")").length)
                        {
                            ...
                            var summ_td = parseInt(delSpaces($(newBasket).find(".tabs_content li[item-section="+$(element).attr("item-section")+"] .summ-cell").text()).replace(/,/g, ""));
                            $(this).find("tbody tr[data-id!=total_row] td.summ-cell").stop().animateNumbers(summ_td, correctSpeed, true);
                            $(this).find("tbody tr[data-id!=total_row] .cost-cell .price:not(.discount)").stop().animateNumbers(parseInt(delSpaces($(newBasket).find(".tabs_content li[item-section="+$(element).attr("item-section")+"] .cost-cell .price:not(.discount)").text())), 333, true);
                            $(this).find("tbody tr[data-id!=total_row] .cost-cell .price.discount strike").stop().animateNumbers(parseInt(delSpaces($(newBasket).find(".tabs_content li[item-section="+$(element).attr("item-section")+"] .cost-cell .price.discount strike").text())), 333, true);
                        }
