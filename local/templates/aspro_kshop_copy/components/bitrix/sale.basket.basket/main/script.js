...
function preAnimateResult(basketId, action, direction, correctSpeed, itemSection) {
    ...
    if (typeof basketId != "undefined") {
        ...
        var rowPrice = parseFloat(delSpaces($(document).find("#basket_form [data-id=" + basketId + "] .summ-cell").text()).replace(",", ".")).toFixed(2);  
        var itemPrice = parseFloat(delSpaces($(document).find("#basket_form [data-id=" + basketId + "] .cost-cell .price:not(.discount)").text()).replace(",", ".")).toFixed(2);
        var itemPriceDiscount = parseFloat(delSpaces($(document).find("#basket_form [data-id=" + basketId + "] .cost-cell .price.discount strike").text()).replace(",", ".")).toFixed(2);
        ...
        if (typeof action != "undefined") {
            $(document).find("#basket_form tfoot tr[data-id=total_row] .row_values div[data-type]").each(function(i, element) {			
                if ($(this).attr("data-type") == "price_discount") {
                    var resultSummAll = parseFloat(delSpaces($(this).find("span.price").text()).replace(",", ".")).toFixed(2);
                    var resultSummDiscount = parseFloat(delSpaces($(this).find("div.price.discount strike").text()).replace(",", ".")).toFixed(2);
                    if (action == "update") {
                        if (direction == 'up') {
                            resultSummAll = resultSummAll * 1 + itemPrice * 1; 
                            if (itemPriceDiscount) { 
                                resultSummDiscount = resultSummDiscount * 1 + itemPriceDiscount * 1; 
                            } else { 
                                resultSummDiscount = resultSummDiscount * 1 + itemPrice * 1; 
                            }		
                        } else {
                            resultSummAll = resultSummAll * 1 - itemPrice * 1;
                            if (itemPriceDiscount) { 
                        ...
                        resultSummAll = resultSummAll * 1 - rowPrice * 1;
                        resultSummDiscount = resultSummDiscount * 1 - rowPrice * 1;
                    } else if (action=="delay") {
                        resultSummAll = resultSummAll * 1 - rowPrice * 1;
                        resultSummDiscount = resultSummDiscount * 1 - rowPrice * 1;
                    } else if (action=="add") {
                        resultSummAll = resultSummAll * 1 + itemPrice*BX("QUANTITY_"+basketId).value;
                        resultSummDiscount = resultSummDiscount * 1 + itemPrice*BX("QUANTITY_"+basketId).value;
                    }
                    if (resultSummAll) { 
                        $(this).find("span.price").animateNumbers(resultSummAll, correctSpeed, true); 
                    }
                    if (resultSummDiscount) { 
                        $(this).find("div.price.discount strike").animateNumbers(resultSummDiscount, correctSpeed, true); 
                    }
    ...
    if (typeof result != "undefined") {
        ...
        if ($(newBasket).length) {			
            ...
            if (action=="update") {
                var arDasketIDs = [];
                if (typeof basketId == "undefined") { 
                    $(oldBasket).find("tbody tr[data-id]").each(function(i, element) { 
                        arDasketIDs.push($(element).attr("data-id")); 
                    }); 
                } else { 
                    arDasketIDs.push(basketId); 
                }

                console.log($(newBasket).find("input[name=COUPON]").val());
                console.log($(oldBasket).find("input[name=COUPON]").val());
                ...
                } else if ($(newBasket).find("input[name=COUPON]").val() 
                    && $(oldBasket).find("input[name=COUPON]").val() 
                    && ($(newBasket).find("input[name=COUPON]").val() == $(oldBasket).find("input[name=COUPON]").val())) { 
                        $(oldBasket).find("input[name=COUPON]").addClass("good").removeClass("error"); 
                }
                $(oldBasket).find("input[name=COUPON]").attr("value", $(oldBasket).find("input[name=COUPON]").val()); 


                $(arDasketIDs).each(function(i, basketId) {
                    var newSummPrice = parseFloat(delSpaces($(newBasket).find("tr[data-id="+basketId+"] .summ-cell").text().replace(",", "."))).toFixed(2);
                    $(oldBasket).find("tr[data-id="+basketId+"] .summ-cell").stop().animateNumbers(newSummPrice, correctSpeed, true);
                    if(parseFloat(delSpaces($(oldBasket).find("tr[data-id="+basketId+"] .discount-cell").text()))
                        != parseFloat(delSpaces($(newBasket).find("tr[data-id="+basketId+"] .discount-cell").text()))) {
                            $(oldBasket).find("tr[data-id="+basketId+"] .discount-cell").stop().animateNumbers(
                                parseFloat(
                                    delSpaces($(newBasket).find("tr[data-id="+basketId+"] .discount-cell").text())
                                ).toFixed(2), 
                                333, 
                                false, 
                                parseFloat(
                                    delSpaces($(oldBasket).find("tr[data-id="+basketId+"] .discount-cell").text())
                                ).toFixed(2), 
                                'swing', 
                                function() {
                                    $(oldBasket).find("tr[data-id="+basketId+"] .discount-cell").text($(oldBasket).find("tr[data-id="+basketId+"] .discount-cell").text() + '%')
                                }
                            );
                        }
                        if(parseFloat(delSpaces($(oldBasket).find("tr[data-id="+basketId+"] .cost-cell .price:not(.discount)").text().replace(",", "."))).toFixed(2) 
                            != parseFloat(delSpaces($(newBasket).find("tr[data-id="+basketId+"] .cost-cell .price:not(.discount)").text().replace(",", "."))).toFixed(2)) {
                                $(oldBasket).find("tr[data-id="+basketId+"] .cost-cell .price:not(.discount)").stop().animateNumbers(parseFloat(delSpaces($(newBasket).find("tr[data-id="+basketId+"] .cost-cell .price:not(.discount)").text().replace(",", "."))).toFixed(2), 333, true);
                        }
                        if(parseFloat(delSpaces($(oldBasket).find("tr[data-id="+basketId+"] .cost-cell .price.discount strike").text().replace(",", "."))).toFixed(2)
                            != parseFloat(delSpaces($(newBasket).find("tr[data-id="+basketId+"] .cost-cell .price.discount strike").text().replace(",", "."))).toFixed(2)) {
                                $(oldBasket).find("tr[data-id="+basketId+"] .cost-cell .price.discount strike").stop().animateNumbers(parseFloat(delSpaces($(newBasket).find("tr[data-id="+basketId+"] .cost-cell .price.discount strike").text().replace(",", "."))).toFixed(2), 333, true);
                        }
                        if($(oldBasket).find("tr[data-id="+basketId+"] .cost-cell .price.discount").length 
                            && !$(newBasket).find("tr[data-id="+basketId+"] .cost-cell .price.discount").length) {
                        ...
                        } else if($(newBasket).find("tr[data-id="+basketId+"] .cost-cell .price.discount").length 
                            && !$(oldBasket).find("tr[data-id="+basketId+"] .cost-cell .price.discount").length) {
                            var discountPrice = parseFloat(delSpaces($(newBasket).find("tr[data-id="+basketId+"] .cost-cell .price.discount strike").text().replace(",", "."))).toFixed(2);
                        ...
                        if ($(newBasket).find("tr[data-id="+basketId+"] .count-cell .error").length) {
                            ...
                            if ($(oldBasket).find("tr[data-id="+basketId+"] .count-cell .error").length) {
                                ...
                                ($(newBasket).find("tr[data-id="+basketId+"] .count-cell .error").html());
                            }
                ...
                if (!$(newBasket).find("tbody tr[data-id="+basketId+"]").length) { 
                ...
                $(oldBasket).find(".tabs_content.basket li").each(function(i, element) {					
                    if (!$(newBasket).find(".tabs_content.basket li:eq("+i+")").length) {
                        ...
                        $(this).find("tbody tr[data-id!=total_row] td.discount-cell").text($(newBasket).find(".tabs_content li[item-section="+$(element).attr("item-section")+"] td.discount-cell").text());
                        var summ_td = parseFloat(delSpaces($(newBasket).find(".tabs_content li[item-section="+$(element).attr("item-section")+"] .summ-cell").text()).replace(",", ".")).toFixed(2);
                        $(this).find("tbody tr[data-id!=total_row] td.summ-cell").stop().animateNumbers(summ_td, correctSpeed, true);
                        $(this).find("tbody tr[data-id!=total_row] .cost-cell .price:not(.discount)").stop().animateNumbers(parseFloat(delSpaces($(newBasket).find(".tabs_content li[item-section="+$(element).attr("item-section")+"] .cost-cell .price:not(.discount)").text()).replace(",", ".")).toFixed(2), 333, true);
                        $(this).find("tbody tr[data-id!=total_row] .cost-cell .price.discount strike").stop().animateNumbers(parseFloat(delSpaces($(newBasket).find(".tabs_content li[item-section="+$(element).attr("item-section")+"] .cost-cell .price.discount strike").text()).replace(",", ".")).toFixed(2), 333, true);
                    }
                    ...
                    var section = $(oldBasket).find(".tabs_content.basket li[item-section="+$(element).attr("item-section")+"]");
                        if ($(section).length) {
                            if (typeof itemSection != "undefined") { 
                                if ($(element).attr("item-section")!=itemSection) { 
                            ...
                            $(oldBasket).find(".tabs_content.basket").append($(element)); 
                        }
                });
            ...
            $(oldBasket).find(".row_values div[data-type]").each(function(e, element) {
                if (!$(results).find(".row_values div[data-type="+$(this).attr("data-type")+"]").length) {
                    if ($(this).attr("data-type")=="price_discount" 
                        && $(results).find(".row_values div[data-type=price_normal]").length) {
                            $(this).attr("data-type", "price_normal");
                            var priceDiscountBlock = $(this).find(".price.discount");
                            $(priceDiscountBlock).fadeOut(200).slideUp(333, function() { 
                                $(priceDiscountBlock).remove(); 
                            });	
                        } else if ($(this).attr("data-type")=="price_normal" 
                            && $(results).find(".row_values div[data-type=price_discount]").length) {
            ...
            $(results).find(".row_values div[data-type]").each(function(e, element) {
                if ($(oldBasket).find("tr[data-id=total_row] .row_values div[data-type="+$(element).attr("data-type")+"]").length) {	
                    ...
                    var newDiscountPrice = parseFloat(delSpaces($(element).find("div.price.discount strike").text()).replace(",", ".")).toFixed(2);	
                        ...
                        if ($(element).attr("data-type")!= "price_discount" 
                            && $(element).attr("data-type") != "price_normal") {
            ...
            if ($(newBasket).find(".cart_empty").length && !$(oldBasket).find(".cart_empty").length) {	
                ...
                $(oldBasket).find(".cart_empty").fadeIn(333).parents("ul").animate({
                    "height" : $(oldBasket).find(".cart_empty").height()
                }, function() {
