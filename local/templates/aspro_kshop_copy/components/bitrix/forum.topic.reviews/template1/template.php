...
foreach ($arResult["MESSAGES"] as $res):
	...
	?><table cellspacing="0" border="0" class="reviews-post-table <?=($iCount == 1 ? "reviews-post-first " : "")?><?
		...
		?>" bx-author-id="<?=$res["AUTHOR_ID"]?>" bx-author-name="<?=$res["LAST_NAME"].' '.$res["NAME"]?>" id="message<?=$res["ID"]?>">
		<thead><tr><td>
		<?
...
<div>   
		...
		if (intVal($res["AUTHOR_ID"]) > 0 && !empty($res["AUTHOR_URL"])):
			?><a href="<?=$res["AUTHOR_URL"]?>"><?=$res["LAST_NAME"].' '.$res["NAME"]?></a><?
		else:
			?><?=$res["LAST_NAME"].' '.$res["NAME"]?><?
		endif;
		?></b>
