...
if(!empty($arResult["AUTH_SERVICES"]))
{
	...
	<div class="soc-serv-main">
		<div class="soc-serv-split-title-grey"><?=GetMessage("SS_GET_COMPONENT_INFO")?></div>
	<?
...
{
	?>
	<div class="soc-serv-title"><?=GetMessage("SS_YOUR_ACCOUNTS");?></div>
			...
			foreach($arResult["DB_SOCSERV_USER"] as $key => $arUser)
			{
				...
				<tr class="soc-serv-personal">
					<td class="bx-ss-icons">
						...
						<?if ($arUser["PERSONAL_LINK"] != ''):?><a class="soc-serv-link" target="_blank" href="<?=$arUser["PERSONAL_LINK"]?>"><?endif;?>
						<?=$authID?>
						<?if ($arUser["PERSONAL_LINK"] != ''):?></a><?endif;?>
					</td>
