...
if(!isFunction("isRealValue")){
    function isRealValue(obj){
        return obj && obj !== "null" && obj!== "undefined";
    }
}

if(!isFunction('reloadTopBasket')){
...
if(!isFunction('onLoadjqm')){
	var onLoadjqm = function(name, hash, requestData, selector){
				...
				else
				{
					$.ajax({
						...
						success: function(data)
						{
                            ...
                            $('.jqmClose, .jqmOverlay').attr('onclick', "window.location.assign('http://'+location.hostname)");
						}
					});
				}
				return false;
		...
		$('.'+name+'_frame').show();
         ...
         $('body').on('click', '.jqmOverlay', function(){
            alert('dfs');
            window.location.assign('http://'+location.hostname);
        });
...
if(!isFunction("oneClickBuy"))
{
	var oneClickBuy = function (elementID, iblockID, that)
	{
		...
		$('body').append('<div class="'+name+'_frame popup"></div>');
         $(".one_click_buy_frame").css("top", "25%");
          $(".one_click_buy_frame").css("left", "33%");
...
if(!isFunction("oneClickBuyBasket"))
{
	var oneClickBuyBasket = function ()
	{
		...
		$('body').find('.'+name+'_frame').remove();
		$('body').append('<div class="'+name+'_frame popup"></div>');
        $(".one_click_buy_basket_frame").css("top", "25%");
          $(".one_click_buy_basket_frame").css("left", "33%");
