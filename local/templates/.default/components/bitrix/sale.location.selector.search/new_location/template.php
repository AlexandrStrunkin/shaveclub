<?if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true)die();
    ...
    if ($arParams["AJAX_CALL"] != "Y"
        ...
        <input onChange="<?=$arParams["ONCITYCHANGE"]?>;" <?=$checked?> type="radio" name="NEW_LOCATION_<?=$arParams["ORDER_PROPS_ID"]?>" value="<?=$val["ID"]?>" id="loc_<?=$val["ID"]?>" /><label for="loc_<?=$val["ID"]?>"><?=$val["LOC_DEFAULT_NAME"]?></label>
        <?endforeach;?>
    ...
    <?endif;?>
<label>
    <input size="<?=$arParams["SIZE1"]?>" name="<?echo $arParams["CITY_INPUT_NAME"]?>_val" id="<?echo $arParams["CITY_INPUT_NAME"]?>_val" value="<?=$arResult["LOCATION_STRING"]?>" class="input" type="text"  autocomplete="off" onfocus="loc_sug_CheckThis(this, this.id);" <?=($arResult["SINGLE_CITY"] == "Y" ? " disabled" : "")?> placeholder="город"/>
</label>    
