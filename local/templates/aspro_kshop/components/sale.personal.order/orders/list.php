...
<div class="module-order-history">
	...
	<ul class="tabs">
		<li <?=$cur_date == 'all' ? 'class="cur"' : ''?>>
			<a href="<?=$arParams["SEF_FOLDER"]?>"><span><?=GetMessage("PERSONAL_ORDER_ALL")?></span></a>
			<div class="triangle"></div>
		</li>
		<?foreach( $arYear as $year ):?>
			<li <?=$cur_date == $year ? 'class="cur"' : ''?>>
				<a href="<?=$arParams["SEF_FOLDER"]?>?filter_date_from=01.01.<?=$year?>&filter_date_to=31.12.<?=$year?>&filter=Y"><span><?=$year?></span></a>
				<div class="triangle"></div>
			</li>
		<?endforeach;?>
	</ul>
	...
	<script>$(".tabs > li").click(function() { if (!$(this).is(".cur")){ $(this).addClass("cur").siblings().removeClass("cur"); }});</script>
</div>