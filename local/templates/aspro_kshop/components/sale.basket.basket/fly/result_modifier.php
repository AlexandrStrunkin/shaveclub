<?
	...
	{
		usort($arResult["GRID"]["ROWS"], 'CKShop::cmpByID');
		$arImages = array();
		...
		foreach($arResult["GRID"]["ROWS"] as $key=>$arItem)
		{
			...
			$arResult["GRID"]["ROWS"][$key]["SUMM_FORMATED"] = CCurrencyLang::CurrencyFormat($arItem["PRICE"]*$arItem["QUANTITY"], $arItem["CURRENCY"]);				
		}
		unset($arImages);
		
		$isPrice =$isSumm=$isDiscount= false;
		$priceIndex = 0;
		...
		/*if(!$isSumm){
			$arResult["GRID"]["HEADERS"][]=array('id' => 'SUM', 'name' =>'');
		}*/
		if(!$isDiscount){
			$arResult["GRID"]["HEADERS"][]=array('id' => 'DISCOUNT', 'name' =>'');
		}
