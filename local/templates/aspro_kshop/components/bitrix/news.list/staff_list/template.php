...
<div class="staff_wrapp">
	<?if($arResult["SECTIONS"]):?>
		<?foreach($arResult["SECTIONS"] as $key => $arSection):?>
			<div class="section">
				<a class="section_title<?=(!$key? ' opened' : '')?>">
					<span class="icon"><i></i></span>
					<span class="pseudo"><span class="item_title"><?=$arSection["NAME"];?></span></span>
				</a>
				<div class="items clearfix" <?if ($key==0):?>style="display: block;"<?endif;?>>
					...
					<?if($arSection["ITEMS"]):?>
						...
						<?endforeach;?>
					<?else:?>
						<p class="no_staff"><?=GetMessage("NO_STAFF");?></p>
					<?endif;?>
				</div>
			</div>
		<?endforeach;?>
...
$(document).ready(function() {
	$(".staff_wrapp a.section_title").click(function() {
