<?
    ...
    if ($arParams["SHOW_SECTIONS_LIST_PREVIEW"]=="Y" && $arParams["SECTIONS_LIST_PREVIEW_PROPERTY"] && $arParams["SECTIONS_LIST_PREVIEW_PROPERTY"]!="DESCRIPTION")
    {					
        $arSectionsID = array();						
        $arSectionsDesc = array();						
        foreach($arResult["SECTIONS"] as $key => $arItems) { $arSectionsID[] = $arItems["ID"]; }
        $db_list = CIBlockSection::GetList(array(), Array('ID'=>$arSectionsID), true, array("ID", $arParams["SECTIONS_LIST_PREVIEW_PROPERTY"]));
        while($section = $db_list->GetNext()) 
        { 
            if ($section[$arParams["SECTIONS_LIST_PREVIEW_PROPERTY"]]) { $arSections[$section["ID"]] = $section[$arParams["SECTIONS_LIST_PREVIEW_PROPERTY"]];  }
        }
        foreach($arResult["SECTIONS"] as $key => $arItems) { $arResult["SECTIONS"][$key]["DESCRIPTION_UF"] = $arSectionsDesc[$arItems["ID"]]; }
    }	
?>
