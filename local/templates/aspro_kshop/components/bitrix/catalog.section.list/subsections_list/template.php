	...
	<?foreach($arResult["SECTIONS"] as $arItem):?>
		...
		<section class="item" id="<?=$this->GetEditAreaId($arItem['ID']);?>">	
			...
			<div class="right-data">
				<div class="item-title"><a href="<?=$arItem["SECTION_PAGE_URL"]?>"><span><?=$arItem["NAME"]?></span></a><?=($arItem["ELEMENT_CNT"] ? '&nbsp;<span class="grey">('.$arItem["ELEMENT_CNT"].')</span>' : '')?></div>
...
<?if($arResult["SECTION"]["DESCRIPTION"]):?>
	<img class="shadow t20 b20" src="<?=SITE_TEMPLATE_PATH?>/images/shadow_bottom.png" />
	<div class="main_description"><?=$arResult["SECTION"]["DESCRIPTION"]?></div>
<?else:?>
	<?$arSection = CIBlockSection::GetList(array(), array( "IBLOCK_ID" => $arResult["SECTION"]["IBLOCK_ID"], "ID" => $arItem["ID"] ), false, array( "ID", "UF_SECTION_DESCR"))->GetNext();?>
	<?if ($arSection["UF_SECTION_DESCR"]):?>
		<img class="shadow t20 b20" src="<?=SITE_TEMPLATE_PATH?>/images/shadow_bottom.png" />
