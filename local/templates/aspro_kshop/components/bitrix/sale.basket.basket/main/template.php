...
<? 
	...
	if (strlen($arResult["ERROR_MESSAGE"]) <= 0)
	{
		...
		{
			foreach($arMenu as $key => $arElement) { if ($_COOKIES["KSHOP_BASKET_OPEN_TAB"]==$arElement["ID"]) { $arMenu[$key]["SELECTED"]=true; } }
		}
	...
	<script>
		...
		$(".tabs > li").live("click", function()
		{
			if (!$(this).is(".cur"))
			{
				...
				$(".tabs_content > li:eq("+$(this).index()+") td").each(function() { $(this).width($(this).width()); });
			}
		});
