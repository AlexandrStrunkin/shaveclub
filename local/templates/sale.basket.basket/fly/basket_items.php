						...
						<?foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
							...
							<?elseif ($arHeader["id"] == "QUANTITY"):?>
								<td class="count-cell" style="vertical-align: top !important; padding-left: 2px; padding-right: 2px;">
									...
									<input type="hidden" id="QUANTITY_<?=$arItem['ID']?>" name="QUANTITY_<?=$arItem['ID']?>" value="<?=(float)$arItem["QUANTITY"]?>" /> 
							...
							<?elseif ($arHeader["id"] == "SUMM"):?>
								<td class="summ-cell"><div class="cost prices"><div class="price"><?=SaleFormatCurrency(($arItem["PRICE"]*$arItem["QUANTITY"]), $arItem["CURRENCY"]);?><?//=$arItem["SUMM_FORMATED"];?></div></div></td>
				...
				<tr data-id="total_buttons" class="bottom_btn">
					<td>
						<div class="basket_close">
							<span class="button30 gradient transparent grey_br sbold close"><span><?=GetMessage("SALE_BACK")?></span></span>
					...
					<?if ($arParams["AJAX_MODE_CUSTOM"]!="Y"):?>
						<td>
							<div class="basket_update clearfix">
								<button type="submit"  name="BasketRefresh" class="button30 gradient grey sbold refresh"><span><?=GetMessage("SALE_REFRESH")?></span></button>
						...
						<?if(!$arError["ERROR"]){?>
							<div class="basket_back">
								<div class="wrap_button">
									<a href="<?=$arParams["PATH_TO_BASKET"]?>" class="button30 gradient transparent sbold back_button"><span><?=GetMessage("GO_TO_BASKET")?></span></a>
								</div>
					...
					<?if ($arParams["SHOW_FULL_ORDER_BUTTON"]=="Y" && !$arError["ERROR"]):?>
						<td>
							<div class="basket_checkout clearfix">
								<button type="submit" value="<?=GetMessage("SALE_ORDER")?>" onclick="checkOut();" name="BasketOrder" class="button30 gradient transparent sbold checkout"><span><?=GetMessage("SALE_ORDER")?></span></button>
						...
						<?if(!$arError["ERROR"]){?>
							<?if ($arParams["SHOW_FAST_ORDER_BUTTON"]=="Y"):?>
								<div class="basket_fast_order clearfix">
									<a onclick="oneClickBuyBasket()" class="button30 gradient short sbold fast_order"><span><?=GetMessage("SALE_FAST_ORDER")?></span></a>
