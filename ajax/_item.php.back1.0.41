	...
	if( $_REQUEST["add_item"] == "Y" )
	{
		...
		if( !empty( $dbBasketItems ) && $dbBasketItems["DELAY"] == "Y" ) { $arFields = array("DELAY" => "N", "SUBSCRIBE" => "N"); CSaleBasket::Update($dbBasketItems["ID"], $arFields); } 
			...
			if($_REQUEST["offers"]=="Y"){
				...
				Add2BasketByProductID( $_REQUEST["item"], $_REQUEST["quantity"], array(), $product_properties );
			}else{
				Add2BasketByProductID( $_REQUEST["item"], $_REQUEST["quantity"] );
			}
		}
	}
	else
